...
$hora_que_comeu_manha = $time->format(' H:i:s');
$data_que_comeu_manha =  $time->format('Y/m/d ');
?>
...
<body>
  <div class="container">
    ...
    <form class="form group d-flex justify-content-center flex-column col-6" method="POST" action="enviar_para_banco.php" class="">
      ...
      <input type="text" class="form-control" name="data" value="<?php echo $data_que_comeu_manha; ?>" hidden>
      <input type="text" class="form-control" name="hora_que_comeu_manha" value="<?php echo $hora_que_comeu_manha; ?>" hidden>
    ...
    $sql = " SELECT * FROM alunos_que_ja_comeu_manha WHERE matricula = '$matricula' AND data_que_comeu_manha  =  DATE(NOW()) ";
    $result = mysqli_query($conn, $sql);
    $conta = mysqli_num_rows($result);
    while ($linha = mysqli_fetch_array($result)) {
      $matricula = $linha['matricula'];
      ...
      $turma = $linha['turma'];
      $curso = $linha['curso'];
      $hora_que_comeu_manha = $linha['horario'];
    }
    ...
    if ($conta == 0) {
      $conn = mysqli_connect($servidor, $dbusuario, $dbsenha, $dbname);
      mysqli_set_charset($conn, "utf8");
      $sql = "INSERT INTO  alunos_que_ja_comeu_manha (matricula, nome, turma, curso, data_que_comeu_manha,  hora_que_comeu_manha) VALUES('$matricula', '$nome', '$turma', '$curso', '$data_que_comeu_manha','$hora_que_comeu_manha' )";
