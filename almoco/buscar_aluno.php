...
$hora_que_almocou = $time->format(' H:i:s');
$data_que_almocou =  $time->format('Y/m/d ');
?>
...
<body>
  <div class="container">
    ...
    <form class="form group d-flex justify-content-center flex-column col-6" method="POST" action="enviar_para_banco.php" class="">
      ...
      <input type="text" class="form-control" name="data_que_almocou" value="<?php echo $data_que_almocou; ?>" hidden>
      <input type="text" class="form-control" name="hora_que_almocou" value="<?php echo $hora_que_almocou; ?>" hidden>
    ...
    $sql = " SELECT * FROM alunos_que_ja_almocou WHERE matricula = '$matricula' AND data_que_almocou  =  DATE(NOW()) ";
    $result = mysqli_query($conn, $sql);
    $conta = mysqli_num_rows($result);
    while ($linha = mysqli_fetch_array($result)) {
      $matricula = $linha['matricula'];
      ...
      $turma = $linha['turma'];
      $curso = $linha['curso'];
      $hora_que_almocou = $linha['hora_que_almocou'];
    }
    ...
    if ($conta == 0) {
      $conn = mysqli_connect($servidor, $dbusuario, $dbsenha, $dbname);
      mysqli_set_charset($conn, "utf8");
      $sql = "INSERT INTO  alunos_que_ja_almocou (matricula, nome, turma, curso, data_que_almocou,  hora_que_almocou) VALUES('$matricula', '$nome', '$turma', '$curso', '$data_que_almocou','$hora_que_almocou' )";
